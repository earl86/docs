...
# Elasticsearch Dashboard With Kibana
...
We are going to create a Elasticsearch Simple Dedicated Cluster in topology mode. Our cluster will be consist of 2 master nodes, 3 data nodes, 2 ingest nodes. Here, we are using Elasticsearch version ( `xpack-7.14.0` ) of Elasticsearch distribution for this demo. To learn more about the Elasticsearch CR, visit [here](/docs/guides/elasticsearch/concepts/elasticsearch/index.md).
...
- `spec.version` - is the name of the ElasticsearchVersion CR. Here, we are using Elasticsearch version `xpack-7.14.0` of Elasticsearch distribution.
...
es-cluster   xpack-7.14.0         Ready    4m32s
```
...
Kind:         Elasticsearch
Metadata:
  Creation Timestamp:  2022-04-20T06:51:48Z
  Finalizers:
    kubedb.com
  Generation:  2
  Resource Version:  804388
  UID:               9af9ada9-c9de-4a8f-98b9-fb79376f4932
Spec:
  Auth Secret:
...
Events:
  ...
  ----    ------      ----   ----             -------
  Normal  Successful  14m    KubeDB Operator  Successfully created governing service
  Normal  Successful  14m    KubeDB Operator  Successfully created Service
  Normal  Successful  14m    KubeDB Operator  Successfully created Service
  Normal  Successful  14m    KubeDB Operator  Successfully created Elasticsearch
  Normal  Successful  14m    KubeDB Operator  Successfully created appbinding
  Normal  Successful  14m    KubeDB Operator  Successfully  governing service
...
pod/es-cluster-data-0     1/1     Running   0          16m
pod/es-cluster-data-1     1/1     Running   0          16m
pod/es-cluster-data-2     1/1     Running   0          16m
pod/es-cluster-ingest-0   1/1     Running   0          16m
pod/es-cluster-ingest-1   1/1     Running   0          16m
pod/es-cluster-master-0   1/1     Running   0          16m
pod/es-cluster-master-1   1/1     Running   0          16m

NAME                        TYPE        CLUSTER-IP      EXTERNAL-IP   PORT(S)    AGE
service/es-cluster          ClusterIP   10.96.103.250   <none>        9200/TCP   16m
service/es-cluster-master   ClusterIP   None            <none>        9300/TCP   16m
service/es-cluster-pods     ClusterIP   None            <none>        9200/TCP   16m

NAME                                 READY   AGE
statefulset.apps/es-cluster-data     3/3     16m
statefulset.apps/es-cluster-ingest   2/2     16m
statefulset.apps/es-cluster-master   2/2     16m

NAME                                            TYPE                       VERSION   AGE
appbinding.appcatalog.appscode.com/es-cluster   kubedb.com/elasticsearch   7.14.0    16m

NAME                               TYPE                       DATA   AGE
secret/es-cluster-archiver-cert    kubernetes.io/tls          3      16m
secret/es-cluster-ca-cert          kubernetes.io/tls          2      16m
secret/es-cluster-config           Opaque                     1      16m
secret/es-cluster-elastic-cred     kubernetes.io/basic-auth   2      16m
secret/es-cluster-http-cert        kubernetes.io/tls          3      16m
secret/es-cluster-transport-cert   kubernetes.io/tls          3      16m

NAME                                             STATUS   VOLUME                                     CAPACITY   ACCESS MODES   STORAGECLASS   AGE
persistentvolumeclaim/data-es-cluster-data-0     Bound    pvc-0389290a-0b01-42ff-9284-0928cdc9dfa6   1Gi        RWO            standard       16m
persistentvolumeclaim/data-es-cluster-data-1     Bound    pvc-d0931ef4-9e6e-4cb5-a04e-e4ad7b9b3442   1Gi        RWO            standard       16m
persistentvolumeclaim/data-es-cluster-data-2     Bound    pvc-a388dfcb-23fb-4d49-8e26-7863bb4a3bf5   1Gi        RWO            standard       16m
persistentvolumeclaim/data-es-cluster-ingest-0   Bound    pvc-544742ea-987a-4c07-b216-9f0b1acddfe7   1Gi        RWO            standard       16m
persistentvolumeclaim/data-es-cluster-ingest-1   Bound    pvc-25fabb2f-2213-4205-8cec-695c3b183948   1Gi        RWO            standard       16m
persistentvolumeclaim/data-es-cluster-master-0   Bound    pvc-1151f1e3-376d-4670-96ed-d280108a1664   1Gi        RWO            standard       16m
persistentvolumeclaim/data-es-cluster-master-1   Bound    pvc-13e1aa75-8778-429d-a5c1-ae74d725d761   1Gi        RWO            standard       16m
...
## Deploy Elasticsearch Dashboard
...
```
Here, we are going to use `es-cluster-dashboard` Service to connect with the database. Now, let’s port-forward the `es-cluster` Service to the port `9200` to local machine:
...
es-cluster-archiver-cert                  kubernetes.io/tls                     3      13m
es-cluster-ca-cert                        kubernetes.io/tls                     2      13m
es-cluster-config                         Opaque                                1      13m
es-cluster-dashboard-ca-cert              kubernetes.io/tls                     2      11m
es-cluster-dashboard-config               Opaque                                1      11m
es-cluster-dashboard-kibana-server-cert   kubernetes.io/tls                     3      11m
es-cluster-elastic-cred                   kubernetes.io/basic-auth              2      13m
es-cluster-http-cert                      kubernetes.io/tls                     3      13m
es-cluster-token-v2bb4                    kubernetes.io/service-account-token   3      13m
es-cluster-transport-cert                 kubernetes.io/tls                     3      13m
```
